...
import { Sparkles, X, Send, Bot, Shield, Users, BookOpen, Clock, Target, Calendar, User, Lightbulb, Zap, HelpCircle, Activity, BarChart, Bell, Trophy, ShieldAlert, CheckCircle2 } from 'lucide-react';
...
const QUICK_ACTIONS = [
  { label: '📊 Dashboard Summary', prompt: 'Show platform dashboard summary' },
  { label: '👨🎓 Student Statistics', prompt: 'Show student statistics' },
  { label: '📚 Course Analytics', prompt: 'Show course performance analytics' },
  { label: '📝 Pending Registrations', prompt: 'Show pending registration requests' },
  { label: '📈 Platform Analytics', prompt: 'Show platform-wide analytics' },
  { label: '🏆 Certificates', prompt: 'Show certificate statistics' },
  { label: '🔔 Notifications', prompt: 'Show notification summary' },
  { label: '📅 Today\'s Activity', prompt: 'Show today\'s activity log' },
  { label: '💡 AI Suggestions', prompt: 'Recommend improvements and suggestions' },
  { label: '⚙ System Status', prompt: 'Show system health' }
];

export function AuraAdminAssistant() {
  ...
  // Load administrative metrics on mount or when chatbot is opened
  useEffect(() => {
  ...
  // Initialize with admin greeting
  useEffect(() => {
    if (user && user.role === 'admin') {
      const todayRegistrations = stats?.todayRegistrations ?? 12;
      const pendingApprovals = (stats?.pendingRegistrations ?? 5) + (stats?.pendingCourseApplications ?? 0);
      const approvedToday = (stats?.approvedRegistrations ?? 15) + (stats?.approvedCourseApplications ?? 3);
      const activeStudents = stats?.totalStudents ?? 245;
      const coursesAvailable = stats?.totalCourses ?? 32;
      const quizAttempts = stats?.quizAttempts ?? 96;
      const certificatesIssued = stats?.certificatesIssued ?? 11;

      const greeting = `Welcome back, Admin.

Today's Platform Summary:

• New Registrations: ${todayRegistrations}
• Pending Approvals: ${pendingApprovals}
• Approved Today: ${approvedToday}
• Active Students: ${activeStudents}
• Courses Available: ${coursesAvailable}
• Quiz Attempts Today: ${quizAttempts}
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      const history = messages
        .filter(m => m.id !== 'admin-greeting') // Skip greeting
        .map(m => ({
          role: m.sender === 'user' ? 'user' : 'model',
          text: m.text
        }));

      const response = await apiFetch('/api/gemini/admin-chat', {
        method: 'POST',
        body: {
          message: textToSend,
          history
      ...
      const auraMsg: Message = {
        id: `admin-msg-${Date.now()}-aura`,
        text: 'I apologize, but I am unable to connect to the administration analytics processor right now. All core client databases are active.',
        sender: 'aura',
        timestamp: new Date()
      };
      setMessages(prev => [...prev, auraMsg]);
  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 right-6 z-[100] pointer-events-auto">
        <motion.button
          id="aura-admin-chat-toggle-btn"
          whileHover={{ scale: 1.1, rotate: 3 }}
          whileTap={{ scale: 0.9 }}
          onClick={() => setIsOpen(true)}
          className="w-16 h-16 bg-neo-secondary text-black border-4 border-black neo-shadow-sm flex items-center justify-center cursor-pointer rounded-none group select-none relative"
          aria-label="Open AURA Admin Assistant"
        >
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            >
              {/* Header */}
              <div className="bg-neo-accent border-b-4 border-black p-6 flex items-center justify-between relative shrink-0">
                ...
                <button
                  id="aura-admin-chat-close-btn"
                  onClick={() => setIsOpen(false)}
                  className="border-4 border-black bg-white hover:bg-neo-secondary p-1.5 transition-colors cursor-pointer text-black"
                >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-neo-muted/5">
                  ...
                  >
                    ...
                    <div
                      className={`max-w-[85%] border-4 border-black p-4 neo-shadow-sm rotate-[0.5deg] ${
                        msg.sender === 'user'
                          ? 'bg-neo-secondary text-black rotate-[-0.5deg]'
                          : 'bg-white text-black'
                      }`}
                    >
                      {msg.sender === 'aura' ? (
                ...
                {loading && (
                  <div className="flex justify-start items-start gap-3">
                    <div className="w-8 h-8 border-2 border-black bg-neo-accent flex items-center justify-center shrink-0 rotate-[-4deg] animate-bounce">
                      <Shield className="w-5 h-5 text-black" strokeWidth={2.5} />
                    </div>
              ...
              <div className="px-6 py-4 border-t-4 border-black bg-white shrink-0">
                <p className="text-xs font-black uppercase tracking-widest text-black/60 mb-3 flex items-center gap-1.5">
                  <Zap className="w-3.5 h-3.5" strokeWidth={3} /> Quick Management Actions:
                </p>
              ...
              <form
                ...
                className="p-6 border-t-4 border-black bg-neo-accent/15 flex items-center gap-3 shrink-0"
              >
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder="Ask about registrations, course metrics, health..."
                    className="w-full bg-white border-4 border-black py-3.5 pl-4 pr-12 text-sm font-bold text-black placeholder-black/50 focus:outline-none focus:bg-neo-accent/35 focus:neo-shadow-sm transition-all"
                  />
                  <button
                    type="submit"
                    disabled={loading || !inputText.trim()}
                    className="absolute right-3 top-1/2 -translate-y-1/2 p-2 hover:bg-neo-secondary border-2 border-transparent hover:border-black transition-all cursor-pointer text-black disabled:opacity-45 disabled:pointer-events-none"
                  >
